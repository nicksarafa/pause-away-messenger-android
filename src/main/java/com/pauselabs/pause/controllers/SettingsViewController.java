...
public class SettingsViewController implements View.OnClickListener {
  ...
  public SettingsViewController() {
    ...
    settingsView.strangersBtn.setContent(
        (prefs.getBoolean(Constants.Settings.REPLY_STRANGERS_KEY, true)) ? "Yes" : "No");
    settingsView.missedCallsBtn.setContent(
        (prefs.getBoolean(Constants.Settings.REPLY_MISSED_CALL_KEY, true)) ? "Yes" : "No");
    settingsView.receivedSmsBtn.setContent(
        (prefs.getBoolean(Constants.Settings.REPLY_SMS_KEY, true)) ? "Yes" : "No");
    settingsView.silentBtn.setContent(
        (prefs.getBoolean(Constants.Settings.PAUSE_ON_SILENT_KEY, true)) ? "Yes" : "No");
    settingsView.vibrateBtn.setContent(
        (prefs.getBoolean(Constants.Settings.PAUSE_ON_VIBRATE_KEY, true)) ? "Yes" : "No");
    //        settingsView.voiceBtn.setContent((prefs.getBoolean(Constants.Settings.PAUSE_VOICE_FEEDBACK_KEY, false)) ? "On" : "Off");
    settingsView.toastBtn.setContent(
        (prefs.getBoolean(Constants.Settings.PAUSE_TOASTS_KEY, false)) ? "On" : "Off");
  ...
  public void changeStrangers(boolean def) {
    ...
    if (def) replyStrangers = Constants.Settings.DEFAULT_REPLY_STRANGERS;
    else
      replyStrangers =
          !prefs.getBoolean(
              Constants.Settings.REPLY_STRANGERS_KEY, Constants.Settings.DEFAULT_REPLY_STRANGERS);

    if (replyStrangers) settingsView.strangersBtn.setContent("Yes");
    else settingsView.strangersBtn.setContent("No");
  ...
  public void changeMissedCalls(boolean def) {
    ...
    if (def) replyMissedCall = Constants.Settings.DEFAULT_REPLY_MISSED_CALL;
    else
      replyMissedCall =
          !prefs.getBoolean(
              Constants.Settings.REPLY_MISSED_CALL_KEY,
              Constants.Settings.DEFAULT_REPLY_MISSED_CALL);

    if (replyMissedCall) settingsView.missedCallsBtn.setContent("Yes");
    else settingsView.missedCallsBtn.setContent("No");
  ...
  public void changeSMS(boolean def) {
    ...
    if (def) replySMS = Constants.Settings.DEFAULT_REPLY_SMS;
    else
      replySMS =
          !prefs.getBoolean(Constants.Settings.REPLY_SMS_KEY, Constants.Settings.DEFAULT_REPLY_SMS);

    if (replySMS) settingsView.receivedSmsBtn.setContent("Yes");
    else settingsView.receivedSmsBtn.setContent("No");
  ...
  public void changeSilent(boolean def) {
    ...
    if (def) pauseOnSilent = Constants.Settings.DEFAULT_PAUSE_ON_SILENT;
    else
      pauseOnSilent =
          !prefs.getBoolean(
              Constants.Settings.PAUSE_ON_SILENT_KEY, Constants.Settings.DEFAULT_PAUSE_ON_SILENT);

    if (pauseOnSilent) settingsView.silentBtn.setContent("Yes");
    else settingsView.silentBtn.setContent("No");
  ...
  public void changeVibrate(boolean def) {
    ...
    if (def) pauseOnVibrate = Constants.Settings.DEFAULT_PAUSE_ON_VIBRATE;
    else
      pauseOnVibrate =
          !prefs.getBoolean(
              Constants.Settings.PAUSE_ON_VIBRATE_KEY, Constants.Settings.DEFAULT_PAUSE_ON_VIBRATE);

    if (pauseOnVibrate) settingsView.vibrateBtn.setContent("Yes");
    else settingsView.vibrateBtn.setContent("No");
  ...
  public void changeVoice(boolean def) {
    ...
    if (def) pauseVoiceFeedback = Constants.Settings.DEFAULT_PAUSE_VOICE_FEEDBACK;
    else
      pauseVoiceFeedback =
          !prefs.getBoolean(
    ...
    prefs
        .edit()
        .putBoolean(Constants.Settings.PAUSE_VOICE_FEEDBACK_KEY, pauseVoiceFeedback)
        .apply();
  }
  ...
  public void changeToast(boolean def) {
    ...
    if (def) pauseToastsOn = Constants.Settings.DEFAULT_PAUSE_SHOW_TOASTS;
    else
      pauseToastsOn =
          !prefs.getBoolean(
              Constants.Settings.PAUSE_TOASTS_KEY, Constants.Settings.DEFAULT_PAUSE_SHOW_TOASTS);

    if (pauseToastsOn) settingsView.toastBtn.setContent("On");
    else settingsView.toastBtn.setContent("Off");
  ...
  public void launchPlayMarketRate() {
    ...
    try {
      ...
      PauseApplication.pauseActivity.startActivity(
          new Intent(
                  Intent.ACTION_VIEW,
                  Uri.parse(
                      "http://play.google.com/store/apps/details?id="
                          + settingsView.getContext().getPackageName()))
  ...
  public void sendFeedbackEmail() {
    ...
    PauseApplication.pauseActivity.startActivity(
        Intent.createChooser(emailIntent, "Contact Üs").addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
  }
  ...
  public void launchSupportLink() {
    Intent termsIntent =
        new Intent(Intent.ACTION_VIEW, Uri.parse("http://www.woot.com"))
            .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
