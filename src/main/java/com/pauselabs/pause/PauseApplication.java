...
public class PauseApplication extends Application {
   ...
   *
   * <p>A single tracker is usually enough for most purposes. In case you do need multiple trackers,
  ...
  public static void checkForSleepMode() {
    if (isPhoneCharging() && isPhoneStill() && isSleepTime())
      startPauseService(Constants.Session.Creator.SLEEP);
        ...
        && getCurrentSession().getCreator() == Constants.Session.Creator.SLEEP)
  ...
  public static boolean isPhoneCharging() {
    return phoneIsCharging;
  }

  public static void setPhoneCharging(boolean isCharging) {
    phoneIsCharging = isCharging;
  }

  public static boolean isPhoneStill() {
    return phoneIsStill;
  }

  public static void setPhoneStill(boolean isStill) {
    phoneIsStill = isStill;
  }

  public static int getOldRingerMode() {
    return oldRingerMode;
  }

  public static void setOldRingerMode(int mode) {
    oldRingerMode = mode;
  }

  public static boolean isDriveModeAllowed() {
    return driveModeAllowed;
  }

  public static void setDriveModeAllowed(boolean isAllowed) {
    driveModeAllowed = isAllowed;
  }
  ...
  public static void updateNotifications() {
    notificationManager.notify(
        Constants.Notification.SESSION_NOTIFICATION_ID, updateMainNotification());
  ...
  public static Notification updateMainNotification() {
    ...
    if (prefs.getBoolean(Constants.Pause.ONBOARDING_FINISHED_KEY, false)) {
      ...
      for (PauseConversation convo : getCurrentSession().getConversations())
        if (convo.getMessagesReceived().size() != 0) num++;
      message =
          (num > 0)
              ? num + ((num == 1) ? " person has" : " people have") + " contacted you."
    ...
    PendingIntent pendingIntent =
        PendingIntent.getActivity(instance, 0, i, PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    PendingIntent editPausePendingIntent =
        PendingIntent.getBroadcast(
            instance, new Random().nextInt(), editPauseIntent, PendingIntent.FLAG_CANCEL_CURRENT);
    ...
    PendingIntent stopPausePendingIntent =
        PendingIntent.getBroadcast(
            instance, new Random().nextInt(), stopPauseIntent, PendingIntent.FLAG_CANCEL_CURRENT);

    // not sleeping intent
    Intent notSleepingPauseIntent = new Intent(instance, NotificationActionListener.class);
    notSleepingPauseIntent.putExtra(
        Constants.Notification.PAUSE_NOTIFICATION_INTENT, Constants.Notification.NOT_SLEEPING);
    PendingIntent notSleepingPausePendingIntent =
        PendingIntent.getBroadcast(
            instance,
            new Random().nextInt(),
            notSleepingPauseIntent,
            PendingIntent.FLAG_CANCEL_CURRENT);

    // not driver intent
    Intent notDriverPauseIntent = new Intent(instance, NotificationActionListener.class);
    notDriverPauseIntent.putExtra(
        Constants.Notification.PAUSE_NOTIFICATION_INTENT, Constants.Notification.NOT_DRIVER);
    PendingIntent notDriverPausePendingIntent =
        PendingIntent.getBroadcast(
            instance,
            new Random().nextInt(),
            notDriverPauseIntent,
            PendingIntent.FLAG_CANCEL_CURRENT);

    // custom intent
    Intent customPauseIntent = new Intent(instance, NotificationActionListener.class);
    customPauseIntent.putExtra(
        Constants.Notification.PAUSE_NOTIFICATION_INTENT, Constants.Notification.MODE_CUSTOM);
    PendingIntent customPausePendingIntent =
        PendingIntent.getBroadcast(
            instance, new Random().nextInt(), customPauseIntent, PendingIntent.FLAG_CANCEL_CURRENT);

    // sleep intent
    Intent sleepPauseIntent = new Intent(instance, NotificationActionListener.class);
    sleepPauseIntent.putExtra(
        Constants.Notification.PAUSE_NOTIFICATION_INTENT, Constants.Notification.MODE_SLEEP);
    PendingIntent sleepPausePendingIntent =
        PendingIntent.getBroadcast(
            instance, new Random().nextInt(), sleepPauseIntent, PendingIntent.FLAG_CANCEL_CURRENT);

    // drive intent
    Intent drivePauseIntent = new Intent(instance, NotificationActionListener.class);
    drivePauseIntent.putExtra(
        Constants.Notification.PAUSE_NOTIFICATION_INTENT, Constants.Notification.MODE_DRIVE);
    PendingIntent drivePausePendingIntent =
        PendingIntent.getBroadcast(
            instance, new Random().nextInt(), drivePauseIntent, PendingIntent.FLAG_CANCEL_CURRENT);
        ...
        notBuilder
            .setContentTitle(
                instance.getString(R.string.app_name)
                    + " "
                    + instance.getString(R.string.pause_session_running_custom))
      ...
      case Constants.Session.Creator.VOLUME:
        notBuilder
            .setContentTitle(
                instance.getString(R.string.app_name)
                    + " "
                    + instance.getString(R.string.pause_session_running_silence))
            .addAction(R.drawable.ic_stat_notificaiton_end, "End", stopPausePendingIntent);
      ...
      case Constants.Session.Creator.SLEEP:
        notBuilder
            .setContentTitle(
                instance.getString(R.string.app_name)
                    + " "
                    + instance.getString(R.string.pause_session_running_sleep))
            .addAction(
                R.drawable.ic_stat_notificaiton_end, "Not Sleeping", notSleepingPausePendingIntent);
      ...
      case Constants.Session.Creator.DRIVE:
        notBuilder
            .setContentTitle(
                instance.getString(R.string.app_name)
                    + " "
                    + instance.getString(R.string.pause_session_running_drive))
            .addAction(
                R.drawable.ic_stat_notificaiton_end, "Not the Driver", notDriverPausePendingIntent);
        break;
  ...
  private static void updateChangeModeNotification() {
    ...
    silencePauseIntent.putExtra(
        Constants.Notification.PAUSE_NOTIFICATION_INTENT, Constants.Notification.MODE_SILENCE);
    PendingIntent silencePausePendingIntent =
        PendingIntent.getBroadcast(
            instance,
            new Random().nextInt(),
            silencePauseIntent,
    ...
    changeModeNotBuilder
        ...
        .setContentIntent(
            PendingIntent.getBroadcast(
                instance, new Random().nextInt(), new Intent(), PendingIntent.FLAG_CANCEL_CURRENT))
        .setPriority(NotificationCompat.PRIORITY_MIN);

    if (getCurrentSession().getCreator() != Constants.Session.Creator.VOLUME)
      changeModeNotBuilder.addAction(
          R.drawable.ic_stat_notificaiton_end, "Silence", silencePausePendingIntent);
    if (getCurrentSession().getCreator() != Constants.Session.Creator.SLEEP)
      changeModeNotBuilder.addAction(
          R.drawable.ic_stat_notificaiton_end, "Sleep", sleepPausePendingIntent);
    if (getCurrentSession().getCreator() != Constants.Session.Creator.DRIVE)
      changeModeNotBuilder.addAction(
          R.drawable.ic_stat_notificaiton_end, "Drive", drivePausePendingIntent);

    notificationManager.notify(
        Constants.Notification.CHANGE_MODE_NOTIFICATION_ID, changeModeNotBuilder.build());
  }
  ...
  public static void handleMessageReceived(PauseMessage receivedMessage) {
    ...
    PauseConversation conversation =
        currentPauseSession.getConversationByContactNumber(receivedMessage.getFrom());
    ...
    if (currentPauseSession.isIced(contactId)) {
      ...
      manager.setStreamVolume(
          AudioManager.STREAM_MUSIC, manager.getStreamMaxVolume(AudioManager.STREAM_MUSIC) / 2, 0);
    ...
    if (currentPauseSession.shouldSenderReceivedBounceback(contactId, receivedMessage.getType())
        && conversation.getMessagesSentFromUser().size() == 0) {
      PauseMessage bounceBackMessage =
          getMessageToBounceBack(receivedMessage.getFrom(), conversation);
      ...
      sendToast(
          "Ignored " + receivedMessage.getTypeString() + " from " + conversation.getContactName());
    }
  ...
  public static void handleMessageSent(PauseMessage sentMessage) {
    ...
    PauseConversation conversation =
        currentPauseSession.getConversationByContactNumber(sentMessage.getTo());
    conversation.addMessage(sentMessage);

    if (conversation.getMessagesSentFromUser().size() == 1)
      PauseApplication.sendToast(
          "I will no longer reply to " + conversation.getContactName() + " until your next Pause.");
  ...
  public static String lookupContact(String contactNumber) {
    ...
    Uri uri =
        Uri.withAppendedPath(
            ContactsContract.PhoneLookup.CONTENT_FILTER_URI, Uri.encode(contactNumber));

    ContentResolver contentResolver = instance.getContentResolver();
    Cursor contactLookup =
        contentResolver.query(uri, new String[] {BaseColumns._ID}, null, null, null);
  ...
  public static void sendToast(
      ...
      final SuperToast.Animations animation) {
  ...
  public static void sendToast(
      ...
      final int textColor) {
    sendToast(
        textToSend, duration, background, animation, textColor, R.drawable.ic_action_pause_on);
  }
  ...
  public static void sendToast(
      ...
      final int icon) {
    sendToast(
        textToSend, duration, background, animation, textColor, icon, SuperToast.IconPosition.LEFT);
  }
  ...
  public static void sendToast(
      ...
      final int textColor,
      final int icon,
      final SuperToast.IconPosition iconPosition) {
    if (prefs.getBoolean(
        Constants.Settings.PAUSE_TOASTS_KEY, Constants.Settings.DEFAULT_PAUSE_SHOW_TOASTS)) {
      new Runnable() {
        @Override
        public void run() {
          ...
          ((TextView) ((LinearLayout) superToast.getView()).getChildAt(0))
  ...
  public static void speak(String textToSpeak) {
    if (prefs.getBoolean(
        Constants.Settings.PAUSE_VOICE_FEEDBACK_KEY,
        Constants.Settings.DEFAULT_PAUSE_VOICE_FEEDBACK)) {
