...
/**
 * Conversations are initiated by an incoming message from someone. Pause bounce back messages are
 * triggered based on Conversation conditions. Conversations will keep track of messages received
...
public class PauseConversation implements Serializable {
  ...
  public ArrayList<PauseMessage> getMessagesReceived() {
    ...
    for (int i = 0; i < messages.size(); i++)
      if (messages.get(i).getType() == Constants.Message.Type.SMS_INCOMING
          || messages.get(i).getType() == Constants.Message.Type.PHONE_INCOMING)
  ...
  public String getStringForBounceBackMessage() {
    ...
    } else if (PauseApplication.getCurrentSession().getCreator()
        == Constants.Session.Creator.VOLUME) {
      messageText = db.getDefaultSave().getString(SavesDatabaseHelper.KEY_TEXT);
    } else {
      String silence = "Silence", drive = "Drive", sleep = "Sleep", modeName = "";

      ArrayList<PauseMessage> messagesReceived = getMessagesReceived();
      int num =
          (messagesReceived.size() <= numberOfStringFiles)
  ...
  private static String getContactName(Context context, String phoneNumber) {
    ...
    Cursor cursor =
        cr.query(uri, new String[] {ContactsContract.PhoneLookup.DISPLAY_NAME}, null, null, null);
