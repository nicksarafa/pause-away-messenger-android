 ...
 *
 * <p>NOTE: The JSON file must be in the assets folder
 *
 * <p>Created by mpollin on 11/7/14.
 */
public class StringRandomizer {
   ...
   *
   * <p>NOTE: without a file this class is useless
   *
   * @param context
   */
  ...
  /**
   * @return A randomized String generated from the given JSON file. Returns null if there is no
   *     JSON file.
   */
  public String getString(String contactName) {
    ...
    for (int i = 0; i < components.size(); i++) {
      ...
      if (i != components.size()) sb.append(' ');
    }
  ...
  protected void parseFile(String filename) throws JSONException, IOException {
    ...
    for (int i = 0; i < in.length(); i++) parseComponent(in.getJSONArray(i));
  }

  public void parseComponent(JSONArray strings) throws JSONException {
    ArrayList<String> component = new ArrayList<String>();
    for (int i = 0; i < strings.length(); i++) component.add(strings.getString(i));
