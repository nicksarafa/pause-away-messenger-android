...
public class PausePhoneStateListener extends BroadcastReceiver {
  ...
  class ReplyPhoneStateListener extends PhoneStateListener {
    ...
    public void onCallStateChanged(int state, String incomingNumber) {
      ...
      int olderSharedPreference =
          sharedPreferences.getInt(Constants.Message.PREFERENCE_OLDER_PHONE_STATE, -1);
      Log.d(TAG, "Old phone state: " + olderSharedPreference);

      if (!incomingNumber.equals("")) {
        sharedPreferences
            .edit()
            .putString(Constants.Message.PREFERENCE_LAST_CALL_NUMBER, incomingNumber)
            .commit();
      }
      ...
      switch (state) {
        case TelephonyManager.CALL_STATE_IDLE:
          if (olderSharedPreference == TelephonyManager.CALL_STATE_OFFHOOK
            ...
            String savedNumber =
                sharedPreferences.getString(Constants.Message.PREFERENCE_LAST_CALL_NUMBER, "none");
