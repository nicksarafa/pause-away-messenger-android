...
public class PauseMessageSender {
          ...
          new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
              ...
              if (!mNetworkInfo.isConnected()) {
                ...
                startMmsTransportSevice(
                    context, recipient, buildMmsMessageBytes(recipient, pauseMessage));
  ...
  /**
   * This function starts the intent service that will be responsible for making the actual HTTP
   * POST. This function is only called after we have verified we are connected to the network and
   * can send an MMS
   *
   * @param context
   * @param bytesToSend
  ...
  /**
   * This function is responsible for drawing the Active Pause message details onto a bitmap The
   * bitmap that this function generates will be the final image to be sent over MMS
   */
  ...
  /**
   * This function is responsible for taking our actual Pause Bounce Back message and converting it
   * into bytes which we can send over the network
   *
   * @param recipient
   * @param pauseBounceBackMessage
   * @return
   */
  public byte[] buildMmsMessageBytes(
      String recipient, PauseBounceBackMessage pauseBounceBackMessage) {
    //Create Pdu Bytes
    ...
    String number =
        ((TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE)).getLine1Number();
