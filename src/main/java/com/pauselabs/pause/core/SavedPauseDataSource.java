...
public class SavedPauseDataSource {
  ...
  public PauseBounceBackMessage createSavedPause(PauseBounceBackMessage pauseMessage) {
    ...
    Cursor cursor =
        database.query(
            MySQLiteHelper.TABLE_SAVED_PAUSES,
            allColumns,
  ...
  public void updateSavedPauseFavorite(PauseBounceBackMessage pauseMessage) {
    ...
    database.update(
        ...
        new String[] {pauseMessage.getId().toString()});
  }
  ...
  public void deleteOldestSavedPauseMessage() {
    Cursor oldestSaveCursor =
        database.rawQuery(
            "SELECT * from pauses ORDER BY " + MySQLiteHelper.COLUMN_CREATED_ON + " ASC LIMIT 1",
            null);
  ...
  public void deleteSavedPauseMessage(PauseBounceBackMessage savedPause) {
    ...
    database.delete(MySQLiteHelper.TABLE_SAVED_PAUSES, MySQLiteHelper.COLUMN_ID + " = " + id, null);
  }
  ...
  public List<PauseBounceBackMessage> getAllSavedPauses() {
    ...
    Cursor cursor =
        database.query(
            ...
            MySQLiteHelper.COLUMN_FAVORITE
                + " DESC, "
                + MySQLiteHelper.COLUMN_CREATED_ON
                + " DESC"); // Order by favorites & newest first
