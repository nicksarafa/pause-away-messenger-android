...
public class MmsSendHelper extends MmsCommunication {
  ...
  private static final String TAG = MmsSendHelper.class.getSimpleName();

  private static byte[] makePost(
      Context context, MmsConnectionParameters.Apn parameters, byte[] mms) throws IOException {
    ...
    try {
      ...
      HttpHost target =
          new HttpHost(targetUrl.getHost(), targetUrl.getPort(), HttpHost.DEFAULT_SCHEME_NAME);
  ...
  public static SendConf sendMms(
      Context context, byte[] mms, String apn, boolean usingMmsRadio, boolean useProxyIfAvailable)
      throws IOException {
