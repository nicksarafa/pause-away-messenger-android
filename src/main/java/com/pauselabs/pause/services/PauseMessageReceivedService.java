...
/**
 * When a Broadcast Receiver (Listener) receives a new message during a Pause Session it will start
 * a PauseMessageReceivedService. This service will be responsible for sending Pause bounce back
 * messages and informing the rest of the app that a new message has been received (through the
 * event bus) so that any subscribed screens can update their UI elements. After this intent has
 * been handled it will stop itself.
 */
public class PauseMessageReceivedService extends IntentService {
  ...
  /**
   * Since PauseMessageReceivedService is only initiated from a PauseSessionService, we can assume
   * that our Pause Application will always have a non-null Session at this point. This function
   * will be used to update the active session by updating or creating a new Conversation based on
   * the message received. The creating or modifying of conversations should happen here, not in
   * Session!
   *
   * @param message
   */
  ...
  private void updateExistingConversation(PauseMessage message) {
    ...
    if (mCurrentPauseSession.shouldSenderReceivedBounceback(mContactId)) {
      if (mCurrentConversation.getMessagesReceived().size()
              == Constants.Pause.SECOND_BOUNCE_BACK_TRIGGER
          && !mCurrentConversation.getSentSecondPause()) {
        // Attempt to send second Pause Bounce Back

        PauseApplication.messageSender.sendSmsMessage(
            message.getSender(), retrieveSecondaryPause());
  ...
  private void createNewConversation(PauseMessage message) {
    ...
    if (mCurrentPauseSession.shouldSenderReceivedBounceback(mContactId)) {
      ...
      if (currentBouncebackMessage.getPathToOriginal() == null
          || currentBouncebackMessage.getPathToOriginal().equals("")) {
         ...
         * secondary pause message at the same time, this will be written to the content provider
         * because its using the SmsManager and this way the conversation makes sense in the users
         * chat history
  ...
  private String lookupSender(String sender) {
    String contactId = "";
    Uri uri =
        Uri.withAppendedPath(ContactsContract.PhoneLookup.CONTENT_FILTER_URI, Uri.encode(sender));
  ...
  private PauseBounceBackMessage retrieveActivePause() {
    PauseBounceBackMessage activePauseMessage =
        PauseApplication.getCurrentSession().getActiveBounceBackMessage();
    return activePauseMessage;
  }

  private PauseBounceBackMessage retrieveSecondaryPause() {
    PauseBounceBackMessage secondaryPause =
        new PauseBounceBackMessage(
            "Pause message", Constants.Pause.SECONDARY_BOUNCE_BACK_MESSAGE_TEXT);
