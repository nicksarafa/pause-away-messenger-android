...
public class PauseMmsTransportService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if(extras != null) {
            ...
            try{
                ...
                SendConf response = sendHelper.sendMms(PauseApplication.getInstance(), bytesToSend, "", true, false);

                if(response.getResponseStatus() == SUCCESS){
                    // write mms to database to be displayed in messaging apps

                    String pathToImage = PauseApplication.getCurrentSession().getActiveBounceBackMessage().getPathToImage();
    ...
    public static Uri insert(Context context, Set<String> recipients, String subject, byte[] imageBytes)
    {
        try
        {
    ...
    /**
     * Retrieve message thread from content resolver for the specified conversation.  Since we have received a message from someone
     * in order to trigger the bounce back, there must be an existing thread
     * @param context
     * @param recipients
     * @return
     */
