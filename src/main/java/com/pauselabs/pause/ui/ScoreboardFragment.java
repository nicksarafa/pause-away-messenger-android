...
public class ScoreboardFragment extends Fragment implements View.OnClickListener {
  ...
  public void initScoreboardUI() {
    ...
    if (mActivePauseBounceBack.getPathToOriginal() != null
        && !mActivePauseBounceBack.getPathToOriginal().equals("")) {
      BitmapDrawable pauseImage =
          new BitmapDrawable(getResources(), mActivePauseBounceBack.getPathToOriginal());
  ...
  public void updateScoreboardUI() {
    if (mActiveSession != null && mActiveSession.isActive() && mActivePauseBounceBack != null) {
      ...
      mScoreboardResponseCount.setText(
          mActiveSession.getResponseCount() + " " + responseMessagesSent);
    } else {
